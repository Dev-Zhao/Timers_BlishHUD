        ...
        private void HandleKeyPressEvents(object sender, KeyboardEventArgs args) {
            ...
            if (EntryRequired || DepartureRequired) {
                ...
                if (Antipode != null && Antipode.Count == 3) {
                    ...
                    double distance = Position.Count == 2
                        ? Math.Sqrt(Math.Pow(x - Position[0], 2) + Math.Pow(y - Position[1], 2))
                        : Math.Sqrt(Math.Pow(x - Position[0], 2) +
                                    Math.Pow(y - Position[1], 2) +
                                    Math.Pow(z - Position[2], 2));
                    playerInArea = distance <= Radius;
                }
